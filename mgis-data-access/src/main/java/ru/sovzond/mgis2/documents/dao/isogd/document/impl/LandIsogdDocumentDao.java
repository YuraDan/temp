 ...
 */
@Repository
public class LandIsogdDocumentDao extends CRUDDaoBase<LandIsogdDocument> implements ILandIsogdDocumentDao {
	public LandIsogdDocument findById(Long id) {
		return (LandIsogdDocument) filter(Restrictions.eq("id", id)).uniqueResult();
	}
	...
	class DocumentBaseBuilder extends PagerBuilderCriteria<LandIsogdDocument> {
	...
	public PagerBuilderCriteria<LandIsogdDocument> createFilter(Volume volume, String order, int first, int max) {
		return new DocumentBaseBuilder(volume, order, first, max);
	}

	public PagerBuilderCriteria<LandIsogdDocument> createSearchDocumentFilter(Section section, String documentName, Date documentDate, Date documentDateFrom, Date documentDateTill, String documentNumber, String orderBy, int first, int max) {
	...
	private class SearchDocumentFilter extends PagerBuilderCriteria<LandIsogdDocument> {

angular.module("mgis.lands.lands", ["ui.router", "ui.bootstrap", "ui.select", "ui.mask", "ngCleanMask", //
	...
	.config(function ($stateProvider) {
		$stateProvider
			.state("lands.lands", {
				url: "/lands/:landId",
				templateUrl: "app2/property/lands/land/land-list.htm"
			})
		;
	}) //
								...
								ExecutivePersonService.get().then(function (executivePersons) {
									...
									NcLandCategoryService.get().then(function (availableLandCategories) {
										...
										NcLandAllowedUsageService.get().then(function (availableLandsAllowedUsage) {
											...
											MGISCommonsModalForm.edit("app2/property/lands/land/land-form.htm", modalScope, function (scope, $modalInstance) {
	...
	.controller("LandsLandItemController", function ($scope, LandsLandItemService) {
		...
		$scope.submenuStates =
			[
				{ name: 'included-objects', url: 'app2/property/lands/land/land-form-included-objects.htm'},
				{ name: 'parent-objects', url: 'app2/property/lands/land/land-form-parent-objects.htm'}
			];
	...
	.controller("LandsLandAreaController", function ($scope, $rootScope, MGISCommonsModalForm, LandsLandAreaTypeService) {
		...
		function editItem(area, addFlag) {
			...
			LandsLandAreaTypeService.get("", 0, 0).then(function (data) {
				...
				MGISCommonsModalForm.edit('app2/property/lands/land/land-area-form.htm', modalScope, function ($scope, $modalInstance) {

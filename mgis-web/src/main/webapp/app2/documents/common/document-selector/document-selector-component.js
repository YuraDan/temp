angular.module("mgis.common.document.selector", ["ui.bootstrap", "ui.select", //
	"mgis.commons",
	...
	.directive("commonDocumentSelector", function ($rootScope) {
		return {
			...
			templateUrl: "app2/documents/common/document-selector/document-selector-component.htm",
				...
				$scope.openSelector = function () {
					...
					function selectionCompleteHandler(selectedDocuments) {
						if ($scope.selectClicked) {
							var result = {};
							if ($scope.multipleDocuments) {
								$scope.documents = result.documents = [].concat(selectedDocuments);
					...
					var modal = MGISCommonsModalForm.edit("app2/documents/common/document-selector/document-selector-form.htm", modalScope, function (scope, $modalInstance) {
	...
	.controller("CommonDocumentSelectorController", function ($scope, ISOGDSectionsService, ISOGDBooksService, ISOGDVolumesService, ISOGDDocumentsService, ISOGDDocumentCRUDService) {

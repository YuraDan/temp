...
public class AddressRESTService implements Serializable {
	...
	@Transactional
	public PageableContainer<Address> list(@RequestParam(value = "orderBy", defaultValue = "") String orderBy, @RequestParam(defaultValue = "0") int first, @RequestParam(defaultValue = "0") int max, @RequestParam(defaultValue = "") String name, @RequestParam(defaultValue = "") String oktmo) {
		return addressBean.list(orderBy, first, max, name, oktmo);
	}
